...
public class KuduGroupScan extends AbstractGroupScan {
  ...
  private void init() {
    ...
    try {
      KuduTable table = storagePlugin.getClient().openTable(tableName);
      KuduScanToken.KuduScanTokenBuilder tokenBuilder = storagePlugin.getClient().newScanTokenBuilder(table);
      if (kuduScanSpec != null && kuduScanSpec.getFilters() != null) {
        for (KuduPredicate predicate : kuduScanSpec.getFilters()) {
          tokenBuilder.addPredicate(predicate);
        }
      }
      List<KuduScanToken> tokens = tokenBuilder.build();
      for (KuduScanToken token : tokens) {
        KuduWork work = new KuduWork(token);
        for (Replica replica : token.getTablet().getReplicas()) {
...
//  }
  ...
  private static class KuduWork implements CompleteWork {
    ...
    private KuduScanToken scanToken;


    public KuduWork(KuduScanToken scanToken) {
      this.scanToken = scanToken;

    }

    public KuduScanToken getKuduScanToken() {
      return scanToken;
    }
...
//  }
  ...
  public KuduSubScan getSpecificScan(int minorFragmentId) {
    ...
    for (KuduWork work : workList) {
      try {
        scanSpecList.add(new KuduSubScanSpec(getTableName(), work.getKuduScanToken().serialize()));
      } catch (IOException e) {
